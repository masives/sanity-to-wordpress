...
const getCategories = async (wpApiUrl) => {
...
module.exports.getWordpressData = async (baseUrl) => {
  ...
  const [users, categories, blogPosts] = await Promise.all([getUsers(wpApiUrl), getCategories(wpApiUrl), getPosts(wpApiUrl, wpMedia)]);
